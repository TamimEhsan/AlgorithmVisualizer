...
import { newNodeId, edgeId } from '@/lib/algorithms/graph';
import GraphNode from '../graph/graph-node';
import FloatingEdge, { EdgeWeightContext } from '../graph/floating-edge';
...
function initialGraph(presetIndex = 0) {
    const { nodes, edges } = toFlow(SP_PRESETS[presetIndex]);
    if (nodes[0]) nodes[0] = { ...nodes[0], data: { ...nodes[0].data, role: 'start' } };
    return { nodes, edges, startId: nodes[0]?.id ?? null };
}

const MODE_HINT = {
    idle: 'Click a node to select · N add node · E add edge · click a weight to edit',
...
function ShortestPathInner() {
    ...
    const handleVisualize = () => {
        ...
        if (algorithmRef.current === 1) {
            const el = edgeList(edgesRef.current, directedRef.current);
            runActions(bellmanFordActions(el, startIdRef.current, finishIdRef.current, ids));
        } else {
            const adj = weightedAdjacency(edgesRef.current, directedRef.current);
            runActions(dijkstraActions(adj, startIdRef.current, finishIdRef.current, ids));
        }
    };
    ...
    const onPresetChange = (idx) => {
        ...
        labelRef.current = 0;
        setNodes(g.nodes);
        setEdges(directedRef.current ? g.edges.map((e) => ({ ...e, markerEnd: ARROW })) : g.edges);
        ...
        <div className="flex flex-col h-screen">
            ...
            <div className="flex flex-1 overflow-hidden">
                ...
                <div className="relative flex-1">
                    ...
                    <EdgeWeightContext.Provider value={setWeight}>
                        <ReactFlow
                            nodes={nodes}
                            edges={edges}
